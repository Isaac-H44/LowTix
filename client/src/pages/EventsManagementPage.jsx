    ...
    async function fetchEvents() {
      ...
      try {
        ...
        if (err.name !== 'AbortError') {
          setError(err.message || 'Failed to load events for admin');
        }
      } finally {
        setLoading(false);
      }
  ...
  return (
    <section className="page">
      <header className="page-header">
        <h1 className="page-title">Admin: Events Management</h1>
        <p className="page-subtitle">
          Read-only admin view of events from the LowTix API. CRUD will be added later.
        </p>
      </header>
          ...
          {events.map((event) => (
            <li key={event.event_id || event.id} className="card">
              ...
              <div className="card-row">
                {event.city}, {event.state} — status: {event.status}
              </div>
            </li>
          ))}
        </ul>
